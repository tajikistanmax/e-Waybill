...
| 4.8 | Автоподстановка одометра выезда из `parkings.indication_counter` (`api/parking_indication_counter`) | `Vehicle.odometer` + `PATCH /vehicles/{id}/odometer`; префилл в форму активации не найден | [~] | Проверить `activate`: подставлять odometerExit из ТС. |
| 4.9 | Автоподстановка «Бақияи пеш аз баромад» = `remain_fuel_entry` ПРЕДЫДУЩЕГО ПЛ того же ТС по виду топлива (`parking_fuel_left`, `ref/remain_fuel`) и «Дода шавад» (`be_given`) из предыдущего ПЛ | нет | [ ] | Эндпоинт «последняя топливная строка ТС по виду топлива» + префилл в форме топлива (карточка ПЛ и кабинет `/fuel`). |
...
| 5.6 | Топливная строка: `be_given` (норма к выдаче), `additional` (харҷи иловагӣ 0..5 л), `coef_below_0` (коэф. темп. ниже 0) — входят в `fuel_calc` | `FuelRecord` без additional/coef_below_0; `WaybillCalcAssembler.fuelLines` передаёт их в `CalcFuelLine` **нулями** | [ ] | **Гэп точности (Шаг 1):** добавить 2 колонки в `fuel_record` (+API/UI/печать), прокинуть в расчёт; `be_given` — вычислять/префилл (4.9). Тест старое-vs-новое с ненулевыми additional/coef_below_0. |
