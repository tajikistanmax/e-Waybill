...
Исходные материалы заказчика (черновики ТЗ, `.docx`, скриншоты старой системы) вынесены из
репозитория 23.09.2026 в папку `D:\Projects\e-Waybill-материалы` рядом с проектом; их содержание
сведено в `spec\`. В истории git они остаются (последний коммит с ними — до `chore(repo): уборка`).
