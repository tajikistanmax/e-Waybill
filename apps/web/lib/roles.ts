...
export function visibleNav(roles: string[]): Set<NavKey> {
  ...
  if (roles.includes('SYSTEM_ADMIN')) add('dashboard', 'waybills', 'med', 'tech', 'company', 'reports', 'dictionaries');
  if (roles.includes('COMPANY_ADMIN')) add('dashboard', 'waybills', 'med', 'tech', 'company', 'reports', 'dictionaries');
