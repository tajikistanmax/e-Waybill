 ...
 */
export function ConfigurableFields({ form, modes, value, onChange, overrides }: {
  form: string;
      ...
      {(FORM_FIELDS[form] ?? []).filter(f => !f.locked && modes[f.key] !== 'hidden').map(f => {
        ...
        return (
          <div key={f.key} className={f.full ? 'full' : undefined}>
            <label>{label}</label>
            {f.type === 'select' ? (
              <select required={required} value={value(f.key)} onChange={e => onChange(f.key, e.target.value)}>
                <option value="">—</option>
                {(f.options ?? []).map(o => <option key={o.v} value={o.v}>{t(o.labelKey)}</option>)}
              </select>
            ) : (
              <input required={required} type={f.type ?? 'text'} value={value(f.key)}
