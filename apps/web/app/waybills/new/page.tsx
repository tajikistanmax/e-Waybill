...
export default function NewWaybillPage() {
  ...
  useEffect(() => {
    ...
    // Направления (Самт) и заказчики — для 2-Б: «Самт» даёт коэффициент нормы топлива и печатается
    // в бланке, заказчик — тоже в бланке (в старой платформе оба заполнены у 100 % грузовых листов).
    // Эти загрузки потерялись при слиянии 29b25e8 — списки в мастере были пустыми (найдено 24.09.2026).
    md.directions().then(setDirections)
