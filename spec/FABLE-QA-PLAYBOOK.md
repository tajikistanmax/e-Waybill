...
| Конкурентность | `& scripts\concurrency-test.ps1` | **4/4 PASS** | инвариант «1 активный ПЛ» под 8-way гонкой + гонка Т2‖Т3 (lost update флагов) |
...
**Среда:**
- **`next build` гнать ТОЛЬКО при остановленных backend-JVM** (порты 8081/8082) — иначе воркеры
  Next зависают/крашатся под конкуренцией за CPU/RAM (признак зависания: CPU node-процесса плоский).
  Без JVM компилируется за ~6-13с. Цикл web-правки: стоп node+8081/8082/3000 → `next build` →
  рестарт сервисов+standalone (копировать .next/static и public в .next/standalone, `node server.js`).
...
- Миграции master-data сейчас до **V16** (V14 брендинг, V15 role_access, V16 security). Следующая — V17.
...
- SMS/push/Telegram — **решение пользователя (12.07.2026): полностью забыть, не планировать и не предлагать.**
  In-app уведомлений достаточно. Интерфейс `NotificationChannel` остаётся как есть, работ по нему нет.
...
**Зелёная база (12.07.2026, после 1-го раунда Fable):** smoke 34/34, security-matrix 26/26,
blocking-checks 5/5, concurrency 4/4; оба сервиса UP; web-сборка 39/39. Раунд 1 закрыл (коммит
`a714872`): replace-guard'ы (BLOCKED/страховка/отдых), орг-принадлежность подписанта титулов,
lost-update гонку confirmMed‖confirmTech (мутации ПЛ теперь через getForUpdate/PESSIMISTIC_WRITE).
