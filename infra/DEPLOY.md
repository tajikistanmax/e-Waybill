...
| `AGGREGATOR_CHANNELS` | разделы API агрегатора: `ref`, `aggregator`, `gps`, `neru` | с 26.09; по умолчанию все четыре. Другие внешние системы (КВД, Smart City) заводятся на странице «Пользователи» → роль «Интегратор» → каналы |
...
- справки (маълумотнома) — `scripts/migration/run_malumotnoma.ps1` (где доступна база legacy):
  674 тарифицированных маршрута справок (без них кассиру не из чего выбирать — в чистой базе
  только 4 демо-маршрута, скрипт их отключает) и архив справок с номерами «Роҳхат». Запускать
  **до начала выдачи справок в e-Waybill** и после последней справки в старой системе: новые справки
  продолжают сквозную нумерацию после последнего перенесённого номера; если номер «Роҳхат» уже занят
  справкой e-Waybill, скрипт останавливается и ничего не пишет. Повторный запуск безопасен (INSERT
  только недостающего). Старые QR справок (`/qrcode/8/…`) открывают перенесённую справку при
  заданном `LEGACY_APP_KEY`.
