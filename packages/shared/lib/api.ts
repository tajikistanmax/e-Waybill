...
// Порядок — как в меню старой платформы (решение владельца 24.09.2026): троллейбус, автобус,
// микроавтобус, легковой (3-С, в т.ч. такси), 2-Б, 5Б-БМ; дальше — виды новой платформы.
// По этому порядку строятся выбор вида ПЛ, фильтры реестров и настройки.
...
export const wb = {
  ...
  activity: (by: 'VEHICLE' | 'DRIVER', from: string, to: string, types?: string[], organizationRma?: string) => fetch(
    ...
    { headers: authHeaders() }).then(r => handle<{ key: string; waybills: number; organizationRma: string | null }[]>(r)),
